...
export const TaskItem = ({ task, onUpdate, onDelete }) => {
  ...
  // Helper functions for due date status
    ...
    return (
      <div className="rounded-2xl border border-white/10 bg-white/5 p-5 shadow-lg">
        ...
        <form onSubmit={handleUpdate} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-white/80">Reminder Time</label>
            <input
              type="datetime-local"
    ...
    <div className={`rounded-2xl border p-5 shadow-lg ${getTaskStatusStyle()}`}>
      ...
      <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm text-white/70">
        <div className="flex items-center gap-2">
          Priority: {task.priority ? <PriorityBadge priority={task.priority} className="ml-1" /> : <span className="font-medium text-white/50">None</span>}
        </div>
        {task.due_date && (
          <div>
            Due: <span className={getDueDateStyles()}>{formatDate(task.due_date)}</span>
            {isOverdue(task) && <span className="ml-2 text-xs text-red-400">(Overdue)</span>}
            {isDueToday(task) && <span className="ml-2 text-xs text-yellow-400">(Today)</span>}
          </div>
        )}
