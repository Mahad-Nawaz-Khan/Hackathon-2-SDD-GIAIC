...
def agent_create_task(
    ...
    Args:
        ...
        reminder_time: Reminder time as relative text like 'in 5 minutes', 'at 3pm', 'tomorrow at 9am' OR YYYY-MM-DD HH:MM format
    ...
    try:
        ...
        # Get user's timezone for reminder parsing
        user = _tool_context.db_session.exec(
            select(User).where(User.id == _tool_context.user_id)
        ).first()
        ...
        if due_date:
            parsed_due_date = _parse_relative_date(due_date, user_timezone)

        # Parse reminder time - handle relative times
        parsed_reminder_time = None
        if reminder_time:
            parsed_reminder_time = _parse_relative_datetime(reminder_time, user_timezone)
            logger.info(f"Parsed reminder time '{reminder_time}' with timezone '{user_timezone}': {parsed_reminder_time}")
        ...
        if existing_tasks:
            ...
            if parsed_reminder_time and parsed_reminder_time != (task.reminder_time):
                update_data["reminder_time"] = parsed_reminder_time
            ...
            if parsed_reminder_time:
                result += f" Reminder: {parsed_reminder_time.strftime('%Y-%m-%d %H:%M')}"
...
def _parse_relative_date(date_str: str, user_timezone: str = "UTC") -> Optional[datetime]:
    ...
    # All date calculations use the user's timezone
    # Result will be converted to UTC before returning

    # Relative date mappings
    if date_str == "today":
        result = today
        return user_tz.localize(result).astimezone(pytz.UTC).replace(tzinfo=None)
...
def _convert_to_utc(local_datetime: datetime, user_timezone: str) -> datetime:
    ...
    # If the datetime is timezone-aware, convert it
    if local_datetime.tzinfo is not None:
        return local_datetime.astimezone(pytz.UTC).replace(tzinfo=None)

    # If naive, assume it's in user's timezone and convert
    localized = user_tz.localize(local_datetime)
    return localized.astimezone(pytz.UTC).replace(tzinfo=None)

def _parse_relative_datetime_no_tz(datetime_str: str, now: datetime) -> Optional[datetime]:
    """
    Parse relative datetime strings using the provided 'now' as base time.
    This is a helper function that assumes 'now' is already in the correct timezone.
    """
    if not datetime_str:
        return None
    ...
    # Try ISO format first (YYYY-MM-DD HH:MM or YYYY-MM-DDTHH:MM)
    try:
    ...
    # "in X minutes" or "X minutes from now" - supports shorthand like "in 5m", "in 10m"
    match = re.search(r'in (\d+)m\b', datetime_str)
    if match:
        minutes = int(match.group(1))
        result = now + timedelta(minutes=minutes)
        return result.replace(second=0, microsecond=0)

    match = re.search(r'in (\d+) minutes?', datetime_str)
    if match:
        minutes = int(match.group(1))
        result = now + timedelta(minutes=minutes)
        return result.replace(second=0, microsecond=0)

    match = re.search(r'(\d+)m from now', datetime_str)
    if match:
        minutes = int(match.group(1))
        result = now + timedelta(minutes=minutes)
        return result.replace(second=0, microsecond=0)

    match = re.search(r'(\d+) minutes? from now', datetime_str)
    if match:
        minutes = int(match.group(1))
        result = now + timedelta(minutes=minutes)
        return result.replace(second=0, microsecond=0)

    # "in X hours" or "X hours from now" - supports shorthand like "in 2h", "in 30m"
    match = re.search(r'in (\d+)h\b', datetime_str)
    if match:
        hours = int(match.group(1))
        result = now + timedelta(hours=hours)
        return result.replace(second=0, microsecond=0)

    match = re.search(r'in (\d+) hours?', datetime_str)
    if match:
        hours = int(match.group(1))
        result = now + timedelta(hours=hours)
        return result.replace(second=0, microsecond=0)

    # "at X pm/am" - parse time within today or tomorrow
    match = re.search(r'at (\d{1,2}):(\d{2})\s*(am|pm)?', datetime_str)
    if match:
    ...
    # Simple "at Xpm" pattern without minutes
    match = re.search(r'at (\d{1,2})(am|pm)', datetime_str)
    if match:
        hour = int(match.group(1))
    ...
    # "tomorrow at X am/pm" or "tomorrow at X:XX"
    if "tomorrow at" in datetime_str:
        tomorrow = now + timedelta(days=1)
        time_part = datetime_str.split("tomorrow at")[1].strip()
        ...
        match = re.search(r'(\d{1,2})(am|pm)', time_part)
        if match:
            hour = int(match.group(1))
            meridiem = match.group(2)
    ...
    # "today at X am/pm" or "today at X:XX"
    if "today at" in datetime_str:
        today_start = now.replace(hour=0, minute=0, second=0, microsecond=0)
        time_part = datetime_str.split("today at")[1].strip()
        ...
        if match:
            ...
            result = today_start.replace(hour=hour, minute=minute)
            if result < now:
                # If the time has passed today, still use today (user specified today)
                pass
            return result
...
def _parse_relative_datetime(datetime_str: str, user_timezone: str = "UTC") -> Optional[datetime]:
    """Parse relative datetime strings like 'in 5 minutes', 'at 3pm', 'tomorrow at 9am'."""
    if not datetime_str:
        return None
    ...
    logger.info(f"Parsing '{datetime_str}' with user timezone '{user_timezone}'. Current time in user TZ: {now}")

    # Parse the datetime in user's timezone
    result = _parse_relative_datetime_no_tz(datetime_str, now)

    if result:
        logger.info(f"Parsed result (in user TZ): {result}, tzinfo: {result.tzinfo}")

        # Convert to UTC and remove timezone info
        # The result might be timezone-aware or naive, so handle both cases
        if result.tzinfo is not None:
            # Has timezone info - convert to UTC and remove tzinfo
            utc_result = result.astimezone(pytz.UTC)
            naive_result = utc_result.replace(tzinfo=None)
            logger.info(f"Converted to UTC: {utc_result}, naive: {naive_result}")
            # Return as naive datetime
            return naive_result
        else:
            # Already naive - assume it's already in UTC or doesn't need conversion
            logger.info(f"Result is already naive: {result}")
            return result
...
def agent_update_task(task_id: int, title: str = "", description: str = "", priority: str = "", due_date: str = "", reminder_time: str = "", completed: bool = None, tags: str = "") -> str:
    """
    ...
    Args:
        ...
        due_date: New due date as relative text (optional)
        reminder_time: New reminder time as relative text (optional)
        ...
        try:
            ...
            self._agent = Agent(
                ...
                instructions=(
                    ...
                    "REMINDERS:\n"
                    "- You CAN set reminders for tasks! Use the reminder_time parameter in agent_create_task\n"
                    "- Reminder formats: 'in 5 minutes', 'in 1 hour', 'at 3pm', 'tomorrow at 9am', 'today at 2:30pm'\n"
                    "- Reminders will send notifications at the specified time\n"
                    "- Example: agent_create_task(title='Meeting', reminder_time='in 5 minutes')\n\n"
                    "TAGS:\n"
                    ...
                    "TASK CREATION:\n"
                    "- Use agent_create_task with: title (required), description, priority, due_date, reminder_time, recurrence, tags\n"
                    "- Example: agent_create_task(title='Buy groceries', due_date='tomorrow', tags='shopping')\n"
                    "- Example with reminder: agent_create_task(title='Call Mom', reminder_time='in 10 minutes')\n"
                    ...
                    "- ALWAYS use the proper parameters: tags, recurrence, priority, reminder_time\n"
                    ...
                    "- When users ask for reminders or say 'remind me to...', ALWAYS use the reminder_time parameter!\n\n"
    ...
    def classify_intent(self, message: str) -> IntentDetectionResult:
        ...
        intent_patterns = {
            IntentTypeEnum.CREATE_TASK: [
                ...
                r'\b(remind\s+me\s+(to|about|in))',
                r'\b(need\s+to|should|have\s+to|gotta)\s+',
                r'\b(set\s+(a\s+)?)?(reminder|alarm)\s+(for|to)',
            ],
